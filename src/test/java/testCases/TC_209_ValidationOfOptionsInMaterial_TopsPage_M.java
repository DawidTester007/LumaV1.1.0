...
public class TC_209_ValidationOfOptionsInMaterial_TopsPage_M extends BaseTest{
	@Test(groups = {"functional","regression","master"})
	public void ValidationOfOptionsInMaterial_TopsPage_M() throws InterruptedException {
		try {
			MainMenuPage Mmp = new MainMenuPage(driver);
			logger.info("****Starting TC_209_ValidationOfOptionsInMaterial_TopsPage_M****");
			Mmp.click_Men_Tops();
			logger.info("****Clicked a category Men ----> Tops****");
			MenTopsPage Mtp = new MenTopsPage(driver);
			Mtp.click_optionShopping_material();
			logger.info("****Clicked matrial tab in shopping options****");
			...
			for(int i =1 ;i<=numberOfCategories ; i++) {
				...
				logger.info("**** "+subCatName+" option of material has been selected****");
				...
				if(Mtp.check_txt_communicateOfAddedOption()==true) {
					...
					Mtp.click_optionShopping_material();
					logger.info("****Clicked material tab in shopping options****");
					
				}else {
					logger.error("****Products have not been sorted by "+subCatName+" ****");
					logger.info("TC_209_ValidationOfOptionsInMaterial_TopsPage_M finished negative");
					Assert.fail("****Products have not been sorted  ****");
				}
			}
			
			logger.info("****Products have been sorted by each option in material****");
			logger.info("****TC_209_ValidationOfOptionsInMaterial_TopsPage_M finished positive****");
