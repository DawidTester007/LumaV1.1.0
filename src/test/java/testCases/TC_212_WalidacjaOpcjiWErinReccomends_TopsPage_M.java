...
public class TC_212_WalidacjaOpcjiWErinReccomends_TopsPage_M extends BaseTest{
	@Test(groups = {"functional","regression","master"})
	public void WalidacjaOpcjiWErinReccomends_TopsPage_M()  {
		try {
			MainMenuPage Mmp = new MainMenuPage(driver);
			logger.info("****Rozpoczęto TC_212_WalidacjaOpcjiWErinReccomends_TopsPage_M****");
			Mmp.click_Men_Tops();
			...
			logger.info("****Przewinięto stronę na górę****");
			Mtp.click_optionShopping_RecErin();
			logger.info("****Kliknięto opcję : Erin Reccomends****");
			...
			if(Mtp.check_txt_communicateOfAddedOption()) {
				...
				logger.info("****Pojawił się komunikat o poprawnym wybraniu Erin Reccomends****");
				...
				logger.info("****TC_212_WalidacjaOpcjiWErinReccomends_TopsPage_M zakończono niepowodzeniem****");
				Assert.fail();
			}
			...
			logger.info("****TC_212_WalidacjaOpcjiWErinReccomends_TopsPage_M zakończono powodzeniem****");
