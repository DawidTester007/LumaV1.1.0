...
public class TC_069_TextValidationOtherProducts_PageProduct extends BaseTest{

	@Test(groups={"functional","master"})
	public void TextValidationOtherProducts_PageProduct() {
	
		try {
		MainPage Mp = new MainPage(driver);
		logger.info("**** Starting TC_069_TextValidationOtherProducts_PageProduct ****");
		...
		if(Pp.get_txt_anotherProducts1().isDisplayed()) {
			...
			if(Pp.get_txt_anotherProducts1().getText().equals(expTxt1)) {
				...
				logger.info("**** TC_069_TextValidationOtherProducts_PageProduct finished positive ****");
				Assert.assertTrue(true);
				
			}else {
				logger.error("**** The content of communicate is incorrect ****" );
				logger.info("**** TC_069_TextValidationOtherProducts_PageProduct finished negative ****");
