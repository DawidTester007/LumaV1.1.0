...
public class TC_215_WalidacjaOpcjiWPattern_TopsPage_M extends BaseTest{
	@Test(groups = {"functional","regression","master"})
	public void WalidacjaOpcjiWPattern_TopsPage_M() throws InterruptedException {
		try {
			MainMenuPage Mmp = new MainMenuPage(driver);
			logger.info("****Rozpoczęto TC_215_WalidacjaOpcjiWPattern_TopsPage_M****");
			Mmp.click_Men_Tops();
			logger.info("****Kliknięto men ---> tops****");
			MenTopsPage Mtp = new MenTopsPage(driver);
			Mtp.click_optionShopping_pattern();
			...
			logger.info("****Przewinięto stronę na górę****");
			Mtp.click_optionShopping_pattern();
			logger.info("****Kliknięto opcję : Pattern****");
			...
			if(Mtp.check_txt_communicateOfAddedOption()) {
				...
				logger.info("****Pojawił się komunikat o poprawnym wybraniu opcji w kategorii: Pattern****");
				Mtp.click_btn_clearOption();
				logger.info("****Kliknięto przycisk wyczyść wszystko****");
			}else {
				
				logger.error("****Nie pojawił się komunikat o poprawnym wybraniu w kategorii : Pattern****");
				logger.info("****TC_215_WalidacjaOpcjiWPattern_TopsPage_M zakończono niepowodzeniem****");
				Assert.fail();
			}
			...
			logger.info("****TC_215_WalidacjaOpcjiWPattern_TopsPage_M zakończono powodzeniem****");
