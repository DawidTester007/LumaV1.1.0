...
public class TC_208_ValidationOfOptionsInPrices_TopsPage_M extends BaseTest{
	@Test(groups = {"functional","regression","master"})
	public void ValidationOfOptionsInPrices_TopsPage_M() throws InterruptedException {
		try {
			MainMenuPage Mmp = new MainMenuPage(driver);
			logger.info("****Starting TC_208_ValidationOfOptionsInPrices_TopsPage_M****");
			Mmp.click_Men_Tops();
			logger.info("****Clicked a category Men ----> Tops****");
			MenTopsPage Mtp = new MenTopsPage(driver);
			Mtp.click_optionShopping_price();
			logger.info("****Clicked prices tab in shopping options****");
			...
			for(int i =1 ;i<=numberOfCategories ; i++) {
				...
				logger.info("**** "+subCatName+" option of prices has been selected****");
				...
				if(Mtp.check_txt_communicateOfAddedOption()==true) {
					...
					logger.info("TC_208_ValidationOfOptionsInPrices_TopsPage_M finished negative");
					Assert.fail("****Products have not been sorted  ****");
				}
			}
			
			logger.info("****Products have been sorted by each option in prices****");
			logger.info("****TC_208_ValidationOfOptionsInPrices_TopsPage_M finished positive****");
