...
public class TC_207_ValidationOfOptionsInStyles_TopsPage_M extends BaseTest{
	@Test(groups = {"functional","regression","master"})
	public void ValidationOfOptionsInStyles_TopsPage_M() throws InterruptedException {
	
		try {
			MainMenuPage Mmp = new MainMenuPage(driver);
			logger.info("****Starting TC_207_ValidationOfOptionsInStyles_TopsPage_M****");
			Mmp.click_Men_Tops();
			logger.info("****Clicked a category Men ----> Tops****");
			MenTopsPage Mtp = new MenTopsPage(driver);
			Mtp.click_optionShopping_style();
			logger.info("****Clicked styles tab in shopping options****");
			...
			for(int i =1 ;i<=numberOfCategories ; i++) {
				...
				logger.info("**** "+subCatName+" option of styles has been selected****");
				...
				if(Mtp.check_txt_communicateOfAddedOption()==true) {
					...
					Mtp.click_optionShopping_style();
					logger.info("****Clicked style tab in shopping options****");
					
				}else {
					logger.error("****Products have not been sorted by "+subCatName+" ****");
					logger.info("TC_207_ValidationOfOptionsInStyles_TopsPage_M finished negative");
					Assert.fail("****Products have not been sorted  ****");
				}
			}
			
			logger.info("****Products have been sorted by each option in styles****");
			logger.info("****TC_207_ValidationOfOptionsInStyles_TopsPage_M finished positive****");
