...
public class TC_212_ValidationOfOptionsInErinReccomends_TopsPage_M extends BaseTest{
	@Test(groups = {"functional","regression","master"})
	public void WalidacjaOpcjiWErinReccomends_TopsPage_M()  {
		try {
			MainMenuPage Mmp = new MainMenuPage(driver);
			logger.info("****Starting TC_212_ValidationOfOptionsInErinReccomends_TopsPage_M****");
			Mmp.click_Men_Tops();
			logger.info("****Clicked a category Men ----> Tops****");
			MenTopsPage Mtp = new MenTopsPage(driver);
			Mtp.click_optionShopping_RecErin();
			logger.info("****Clicked Erin Recommends tab in shopping options****");
			...
			for(int i =1 ;i<=numberOfCategories ; i++) {
				...
				logger.info("**** "+subCatName+" option of Erin Recommends has been selected****");
				...
				if(Mtp.check_txt_communicateOfAddedOption()==true) {
					...
					logger.info("TC_212_ValidationOfOptionsInErinReccomends_TopsPage_M finished negative");
					Assert.fail("****Products have not been sorted  ****");
				}
			}
			
			logger.info("****Products have been sorted by each option in Erin Recommends****");
			logger.info("****TC_212_ValidationOfOptionsInErinReccomends_TopsPage_M finished positive****");
