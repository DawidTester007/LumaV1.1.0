...
public class TC_206_ValidationOfFunctionalitySortByCategory_TopsPage_M extends BaseTest{
	@Test(groups = {"functional","regression","master"})
	public void ValidationOfFunctionalitySortByCategory_TopsPage_M() throws InterruptedException {
		
		
		try {
		MainMenuPage Mmp = new MainMenuPage(driver);
		logger.info("****Starting TC_206_ValidationOfFunctionalitySortByCategory_TopsPage_M****");
		Mmp.click_Men_Tops();
		logger.info("****Clicked a category Men ----> Tops****");
		MenTopsPage Mtp = new MenTopsPage(driver);
		Mtp.click_optionShopping_category();
		logger.info("****Clicked category tab in shopping options****");
		...
		logger.info("**** "+subCatName+" Option of category has been selected****");
		if(Mtp.check_txt_communicateOfAddedOption()==true) {
			...
			logger.info("TC_206_ValidationOfFunctionalitySortByCategory_TopsPage_M finished negative");
			Assert.fail("****Products have not been sorted  ****");
		}
		
		}
		
		logger.info("****Products have been sorted by each option in category****");
		logger.info("****TC_206_ValidationOfFunctionalitySortByCategory_TopsPage_M finished positive****");
